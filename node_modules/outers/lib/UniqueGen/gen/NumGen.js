 ...
 */
function GenerateNumber(length, Numbers) {
    // Final ID Array
    const FinalID = []; // Final ID Array
    while (length > 0) {
        const FinalRandomNumber = GenerateNumberIndex(Numbers); // Generate Random Number Index Between 0 and Array Length - 1
    ...
    const FinalIDString = FinalID.join(""); // Join the Array to String
    const FinalIDNumber = Number(FinalIDString); // Convert the String to Number
    return FinalIDNumber; // Return the Final ID
}
exports.default = GenerateNumber;
// Generate Random Number Index
function GenerateNumberIndex(ArrayNumbers) {
    const TempRandomNumBer = Math.random() * (ArrayNumbers.length - 1 - 0) + 0; // Generate Random Number Between 0 and Array Length - 1
...
//# sourceMappingURL=NumGen.js.map